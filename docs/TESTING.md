...
| FAM1 | 2026-09-14 | agent | ✓ re-run **on an account holding nothing else**, which is the strongest form: with only these two audits on file, the absence of a duplicate hero cannot be confused with anything. Both bills name Matthew as GUARANTOR; the audits stored **Sarah L. Testpatient** and **Matthew T. Testpatient** from the patient field. Each report **$85.00 / $32.00 / $0.00 / $85.00**, one `billed_vs_allowed_mismatch` high confidence. Dashboard: "TESTVILLE FAMILY MEDICINE ASSOCIATES · **2 bills** · **$170.00**", rows named **Sarah** and **Matthew**, **no hero anywhere on the page** |
| FAM4 | 2026-09-14 | agent | ✓ re-run. Guard fired with the wrong-person wording, naming both: "⚠️ **This EOB is for someone else.** The bill is for **Matthew T. Testpatient**, but this statement covers **Sarah L. Testpatient**. Charges here will look 'missing from the EOB' simply because they sit on a different statement. Check you paired the right EOB before acting on the total below." **$85.00 / $0.00 / $85.00 / $85.00** — the `not_in_eob` signature it exists to explain. It correctly **outranks** the generic mispair copy, which would have been false here: same payer, same provider, same code and adjacent dates, so the two documents genuinely are related. The model also caught it independently — "…is not present on the EOB provided, which explicitly states that it covers claims for Sarah L. Testpatient only" |
| IMG1 | 2026-09-14 | agent | ✓ **automated — it is now tier 1, not a checklist.** The plan was verified by calling `extractText` on each file by hand and writing the answers down; `test/browser/images.test.js` does exactly that in real Chromium on every `npm test`. Three tests: every accepted image becomes exactly one render with an empty text field; rotated and upside-down are **accepted, not refused** (orientation is the reviewer's call before an audit is spent); and HEIC is refused with the message naming Settings → Camera → Formats, asserted against the string read out of `extract.js` so the test cannot drift from what a member sees. It also pins that Chrome still reports `application/octet-stream` for .heic — the route that used to reach the useless generic message |
| M7 | 2026-09-14 | agent | ✓ re-run **without a plan on file**, which is the only way to see this path. t4 and t5 gave the t-series shape; after t5 the coverage section showed the 💡 banner — "Your insurer mentioned a benefit limit for **90837 — Psychotherapy, 60 minutes** ('Plan note: 5 of 6 covered outpatient mental health visits used this plan year.')" with **Track it**. **One click, no form**, and Firestore confirms it stored fully configured: `label: "Psychotherapy, 60 minutes"`, `codes: ["90837"]`, `limit: 6`, `planYearStartMonth: 1`, **`source: "remark"`** — and it back-counted the two existing audits to 2/6. After t6: deductible **$720.00 of $1,500.00**, "As stated on your most recent EOB (2026-06-10)", as documented. Tracker reads **3/6, not red** — correct for this subset, since only t4-t6 were run and the red assertion assumes the full t1-t6 series |
...
| X1 | 2026-09-14 | agent | ✓ **five of six paths and all three dismissal routes**, all through Claude in Chrome — which an earlier note wrongly said was impossible. delete an audit · remove the plan (counting "10 limits" correctly) · stop tracking a limit · **erase everything** · **replace with an older plan**, which correctly renders its confirm **teal, not red**, because it is not destructive ("The plan on file covers 2026-01-01 to 2026-12-31. This document covers 2025-01-01 to 2025-12-31, which is earlier."). **Cancel**, **Esc** and **backdrop** each declined with `ROUND TRIP: EXACT` after. Only "delete a saved EOB" is unrun |
...
| FAM2 | 2026-09-14 | agent | ✓ re-run against `cms-2025` ($500 individual / $1,000 **family**) with FAM1's two audits on file. Deductible card reads **$640.00 of $1,000.00** sourced "**Family target from your plan (SBC).**" — it took the FAMILY figure rather than measuring against $500 and reporting the EOB's correct family number as a conflict, which is the defect this plan exists for. Out-of-pocket card agrees: $640.00 of $5,000.00 · 13%, "Family target from your plan (SBC)" |
| FAM3 | 2026-09-14 | agent | ✓ re-run. `matthew-bill.pdf` + `family-eob.pdf` — stems "matthew" and "family", deliberately unmatched — paired as "**1 audit: 1 bill+EOB pair**" and the audit completed, stored against **Matthew T. Testpatient** |
| FAM5 | 2026-09-14 | agent | ✓ **step 1 run for the first time this pass.** A manual tracker over the shared code 90686, limit 2, against Matthew's and Sarah's audits: badge **1 / 2** — the highest single member, **not** the household total of 2 — with the per-person block "**Sarah L. Testpatient** 1 of 2 · **Matthew T. Testpatient** 1 of 2", the context line "Counted per person — a plan's visit limit usually applies to each member, not the household. **2 across everyone**", and the level following the per-member figure ("One covered visit left", not "limit reached"). Pooled it would read 2/2 and tell both of them further care is their responsibility. Step 2 (one-person account shows no per-person block) verified 2026-09-13 |
| S2 | 2026-09-14 | agent | ✓ re-run, **both documents as PNGs**. Result **$2,115.00 / $841.75 / $186.35 / $822.15** with the same three findings (duplicate $145.50, billed-above-allowed $658.65, cost-share math $18.00) — **identical to E1's digital-PDF run of the same documents, to the cent**. That equivalence is the whole point of the plan |
| S3 | 2026-09-14 | agent | ✓ re-run, **with one difference worth watching**. Printed figures matched the text extraction: plan name Acme Silver PPO, plan year 2026-01-01 → 2026-12-31, deductible **$1,500 / $3,000**, both limits (mental health 6, rehabilitation 20). But **`outOfPocketMax` came back empty**, where today's text run of the same document (M1) extracted $6,000 / $12,000. The 2026-08-31 note says every *printed* figure matched and only inferred codes varied; an out-of-pocket maximum is printed. One sample, so model variance is not ruled out — but it is the first time a printed figure has gone missing on the scan path, and worth a second run before trusting it |
| S4 | 2026-09-14 | agent | ✓ re-run, five PNGs at 110dpi. Server log confirms the extraction read the scans correctly — `plan candidates [{"name":"Insurance Company 1: Plan Option 1","ded":[500,1000],"rows":22}]`. Against P1's text run of the same document: plan name, plan year (2025-01-01 → 2025-12-31) and deductible ($500 / $1,000) **identical**; limits **5 against text's 6**, which is the inferred half and expected to vary. **It also proved two other things by accident:** cms-2025 is a 2025 plan replacing a 2026 one, so it raised X1's fifth dialog, and confirming it left **`planCount` at 2 — no second model call** (`pendingplan.js` on production) |
| S1 | 2026-09-14 | agent | ✓ covered by S2's run — the bill in that pair is `scanned-bill-1.png`, so the scan-read-by-model path ran with no local OCR and no model download |
| P1 | 2026-09-14 | agent | ✓ `cms-2025` re-run on a clean account. "Insurance Company 1: Plan Option 1", **2025-01-01 → 2025-12-31**, deductible **$500 / $1,000 family**. **6 limits, every one carrying CPT hints** — home health 60 (99500, G0151, G0299, G0300), rehabilitation 60 (97110, 97112, 92507, 97530), habilitation 60, skilled nursing 60, children's eye exam 1 (92002…), children's glasses 1 (V2020, V2784). Six limits became **4 trackers**, which is correct: habilitation and skilled nursing share codes with earlier rows and first wins. Out-of-period proven twice — the card banner "Your plan year ended 2025-12-31 — upload your new SBC." (no "(assumed)", so the end date was printed) and, in a report, "Not checked against your plan — this bill's service dates fall outside your plan year (ended 2025-12-31)." `cms-2019`/`cms-older` not re-run; their 2026-09-09 rows stand |
| G1 | 2026-09-13 | agent+you | **Blocked this pass, not failed.** `meta/guard` writes are refused by the auto-mode classifier as `[Feature Flag Writes]`, and that refusal looks correct: `auditsEnabled=false` is a **global** switch that takes the product down for every user until restored, so it is not something to grant an agent a standing rule for. Last genuine run 2026-09-10 (kill switch 637ms, independence, ceiling across shards). Run it yourself, or grant the rule for one session |
...
| REV1 | 2026-09-14 | agent | ✓ **all four steps now covered.** 1: one-page bill, shown large, **no pills**. 2: five-image SBC → **five numbered pills** (also seen at 24 pills on C6's booklet). 3: a `.txt` bill → **no pills, no page view, the text itself shown** — the one path where text is still what gets sent. 4: a saved EOB says its text was kept and the file was not, so there are no pages (seen on M5's review screen) |
