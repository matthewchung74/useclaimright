...
| E2 | 2026-09-13 | agent | ✗ **found a defect, fixed.** The bill-only note appeared correctly on the form, the audit ran ($175.00 / $0.00 / $0.00 / $0.00, tracker to 2 of 6), and then the report said "No discrepancies found. **The bill and EOB appear consistent.**" — with no EOB. The copy fixed on 2026-09-09 was right and nothing ever reached it: the fresh-audit path tested `!payload.eob`, and `payload.eob` is `asDoc(null)` = `{text:"",images:[]}`, an object, always truthy. So `noEob` was false for every audit ever run. Both paths now derive it from `auditText(data,"eob")`. Guarded by a drift test. **Re-run needed once hosting deploys** |
