...
| M6 | 2026-09-09 | agent | ✓ "This plan is already on file." returned instantly with no "Reading your plan's terms…" spinner — and `planCount` did **not** move, which is the quantitative version of the same claim: the duplicate is caught before the model call |
...
| E3 | 2026-09-09 | agent | ✓ dialog **counted the limits** ("5 limits this plan set up will go with it. Limits you added yourself, or accepted from an EOB, stay"), red primary. After confirming: dashed "No plan on file · Add now" returned, all five SBC trackers went with the plan, and the deductible fell back to EOB-only sourcing — "$560.00 of **$1,500.00** · As stated on your most recent EOB (2026-06-12)", with no scope word, since an EOB-sourced figure has no scope to name |
