...
| FAM1 | 2026-09-13 | agent | ✓ re-run — **the plan that must never regress, and it did not.** Both bills name Matthew as GUARANTOR; the two audits stored **Sarah L. Testpatient** and **Matthew T. Testpatient**, read from the patient field, so the trap held. Dashboard: "TESTVILLE FAMILY MEDICINE ASSOCIATES · **2 bills** · **$170.00**", rows named **Matthew** $85.00 and **Sarah** $85.00, both "Billed above EOB allowed amount", and **no duplicate hero for the pair** — the only hero on the page is D1's psychotherapy one. Edge case (`emma-bill`, duplicate on one statement) not re-run |
