...
| E7 | 2026-09-09 | agent | ✓ run with permission. Returned to onboarding with the **first-visit** skip wording ("Skip for now — audit a bill first"), flipped from the origin-aware "Not now — back to your audits" seen minutes earlier. Erased 13 audits → 0, 8 saved EOBs → 0, 2 trackers → 0, plan → gone. **Counters unchanged**: planCount stayed 3 and fbCount stayed 1, which is the assertion that reset is not a way to buy more audits. The dialog states that itself — "Today's usage counters stay as they are" |
...
| F1 | 2026-09-09 | agent | ✓ bubble → card with Bug/Idea/Other, note sent, card self-closed. **Field-level check now done too** — the plan said it needed the Firebase console, but admin credentials read it directly: the doc carried `category: "idea"` matching the pill, `screen: "bills"`, `auditId: null` (correct from the dashboard), plus email, uid, platform and createdAt |
