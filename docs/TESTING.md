...
|---|---|---|---|
| C4 | | | |
| C5 | 2026-09-13 | agent | ✓ **first run, and it passes the assertion that matters.** A UB-04 claim form as the bill returned **$0.00 / $0.00 / $0.00 / $0.00**, an empty occurrence table, and "Not checked against your plan — **no service dates could be read from this bill**." It did **not** invent a patient responsibility, which a claim form has no field for. It also does not say "this is a claim form, not your bill" — safe, not helpful, and the obvious improvement. **Run as images:** `file_upload` in Claude in Chrome silently fails on both AcroForm claim PDFs (reports success, `#bill-file.files` stays empty, app.js never sees a file; `fake-bill.pdf` from the same click sequence stages fine). Flattened to PNG with `pdftoppm -r 150`, which is also the truer case — a member photographing the form they were handed |
| C6 | 2026-09-13 | agent | ✓ **run end to end.** Removed the plan (E3) so the booklet extracted fresh. Review screen: "Your plan document is **135 pages**. We're sending the **24** that look like the benefits schedule — it covers **3 plans** (EPO Plan, PPO Plan, HDHP Plans)" — **exactly what `booklet.test.js` measures offline**, so the free test predicts production. Extraction returned all three plans (EPO no deductible, PPO $500/$1,500, HDHP $2,500/$5,000) and **pre-selected none**: "Your EOBs disagree about your plan — PPO and HDHP. Choose the one you are on. Until you say, bills are not checked against plan terms." `resolvePlan` refusing to guess, which is what it is for. Account restored, `ROUND TRIP: EXACT` |
...
| E1 | 2026-09-13 | agent | ✓ re-run on the clean account. **$2,115.00 / $841.75 / $186.35 / $822.15** — exact, 145.50 + 18.00 + 658.65 = 822.15. `cost_share_error` spelled out its own arithmetic ($8.24 + $4.55 + $124.00 + $27.70 + $3.86 = $168.35 against a stated $186.35). Occurrence table: 80053 **count 2**, $145.50 ×2. **Honest footer present** — "Not checked against your plan — add your Summary of Benefits under 'Your coverage' on the bills page to enable plan checks." |
...
| M1 | 2026-09-13 | agent | ✓ re-run. Plan on file **Acme Silver PPO 2026-01-01 → 2026-12-31**, no plan-choice list (single-plan SBC). Deductible **$50.00 of $1,500.00** and OOP **$0.00 of $6,000.00 · 0%**, both reading "Individual target from your plan (SBC)." Both trackers auto-created: Rehabilitation **0/20**, Outpatient mental health **0/6**, each badged "from your plan (SBC) — check the codes" |
| M4 | 2026-09-13 | agent | ✓ re-run. Filename routing gave "**2 audits**: 2 bill+EOB pairs" and a "Start 2 audits →" button; ONE review screen with **3 tabs** and "Reviewing t2-bill.pdf — document 1 of 3"; landed on **Bills & coverage**, not a report. **JUST AUDITED · 2 bills · $110.00**, one $55.00 row each (Mar 11, Feb 12), and both bills also present in their provider group below — a lens on the list, not a second list. The consolidated EOB saved **once** for both audits (4 saved EOBs total = one per distinct document) |
| M5 | 2026-09-13 | agent | ✓ re-run. With **4** saved EOBs **nothing was pre-selected** and the label read "Choose which saved EOB covers this bill — the wrong one makes an audit look clean when it isn't". Review screen: "**Using saved EOB:** Testville Behavioral Health Associates · 2026-02-12", tab labelled **EOB (saved)** with "its text was stored… the file itself was not, so there are no pages to show" (REV1 step 4, same screen). Report: **$175.00 / $0.00 / $175.00 / $175.00**, one `not_in_eob` — "does not appear on the provided EOB, which only covers services through 2026-03-11" — and **no** "every line came back missing" warning, which needs 2+. Tracker 5 of 6 with "One covered visit left this plan year" (5 not 4 because E2 ran first in this pass) |
| M6 | 2026-09-13 | agent | ✓ re-run with the **real 135-page booklet**, whose sha256 matches the document already on file. "This plan is already on file." and the skip link read the origin-aware **"Not now — back to your audits"**, not the first-visit wording. **Cost zero plan uploads** — `meta/usage` identical before and after, so the duplicate guard short-circuits ahead of the model call rather than after it |
| D1 | 2026-09-13 | agent | ✓ re-run, needing only one audit: t2 was already on file from M4, so `t2-bill-rebill` + `t2-eob` completed the pair. **The hero fired** — yellow ribbon "FOUND BY COMPARING YOUR BILLS TO EACH OTHER", **$175.00 at stake**, "The same visit is on two statements", naming the provider, **90837** and **Feb 12, 2026**, with STATEMENT A / STATEMENT B each showing its **audited date and amount and no statement number**, plus the "Why this was flagged" explainer. Neither individual report contains it, which is the point |
| M2 | 2026-09-13 | agent | ✓ re-run. **$210.00 / $95.00 / $95.00 / $150.00** (115 + 35). "Billed above EOB allowed amount" $115.00 high confidence; "**Copay doesn't match your plan**" $35.00 medium confidence quoting the SBC row verbatim in mono — `SBC “Row: Rehabilitation services (physical, occupational therapy) — $60 copay/visit, deductible does not apply”`. Tracker line above the cards: "Rehabilitation services: **visit 1 of 20** this plan year". No "not checked" footer |
| M3 | 2026-09-13 | agent | ✓ re-run. The t-series shape exactly — **$175.00 / $120.00 / $120.00 / $55.00** — one `billed_vs_allowed` $55.00 and **no plan-mismatch findings**, which is the point: a consistent claim against a plan on file must produce no false positives. Tracker line "Outpatient mental health services: **visit 1 of 6** this plan year" |
| FAM1 | 2026-09-09 | agent | ✓ **no duplicate hero** for two members sharing clinic, day, code and amount. Group read "2 bills · $170.00", rows named **Sarah** and **Matthew** — which also settles step 4, since both bills name Matthew as guarantor and a guarantor read would have printed him twice. Sarah's audit $85.00 / $32.00 / $0.00 / $85.00, and **no wrong-patient warning**, so FAM4's guard does not false-positive on a correct pair. Run with 1 audit, not 2, by reusing FAM4's Matthew audit as the other half |
...
| M7 | 2026-09-09 | agent | ✓ run the **cheap way** — post-E7 there is no plan and no tracker, so the t5 pair alone fires the remark path for 1 audit instead of 3. Cards carried the t-series shape ($175/$120/$120/**$55**). Banner: "💡 Your insurer mentioned a benefit limit for **90837 — Psychotherapy, 60 minutes** ('Plan note: 5 of 6 covered outpatient mental health visits used this plan year.')" and one click on **Track it** created the tracker fully configured, no form. That remark has words between "covered" and "visits", which is exactly the shape the 2026-08-16 matcher fix allows |
| E2 | 2026-09-13 | agent | ✓ **re-run after the fix deployed, and it holds.** Report now reads "Nothing to dispute on the bill itself. We could not compare it against what your insurer allowed, because there is **no EOB** — that is the strongest check. Run this again when the EOB arrives." $175.00 / $0.00 / $0.00 / $0.00, no EOB-comparison finding types, no "not checked" footer (2026-05-13 is in period). Earlier run this pass caught the defect: `noEob` was derived from `!payload.eob`, and `asDoc(null)` returns an object, so it was false for every audit ever run. **Also fixed here:** the processing screen narrated "Analyzing your bill against the EOB…" in bill-only mode — same family, smaller |
| E3 | 2026-09-13 | agent | ✓ re-run. Dialog counted correctly — "**10 limits** this plan set up will go with it" against 11 trackers, the eleventh being EOB-sourced. Confirmed, and the dialog's promise held: all 10 SBC limits went, **Psychotherapy 8/6 (source `remark`) stayed**, and the deductible card fell back to the EOB's $1,500 and dropped the SBC note |
| E4 | 2026-09-13 | agent | ✓ re-run. `t6-bill.pdf` on the SBC dropzone → "**This doesn't look like a Summary of Benefits.**" in the error slot, onboarding card intact, and the plan already on file was left untouched |
| E5 | 2026-09-13 | agent | ✓ re-run as a **replace**, and it double-verifies the tracker-carryover fix on production. Before: 2 trackers, both Acme Silver PPO's (Rehabilitation 20, Outpatient mental health 6). Replaced with `kaiser-calpers-2026.pdf`; after: **3 trackers, all Kaiser's** (Acupuncture, Skilled nursing care, Chiropractic care) and **both Acme trackers gone**. Before the fix the list would have read 5, with Acme's two still badged "from your plan (SBC)" on a Kaiser plan. Note the 14-page SBC takes **~100s** to extract, against ~20s for the 5-page fixtures — long enough to look hung |
| E6 | 2026-09-13 | agent | ✓ re-run. Paired by filename ("1 audit: 1 bill+EOB pair"), and the report banner fired: "⚠️ **This EOB may not cover this bill.** They share no service dates and no procedure codes. Charges here may be marked 'missing from the EOB' only because your insurer never processed this bill. Check you paired the right EOB before acting on the total below." Totals **$2,115.00 / $0.00 / $845.00 / $2,115.00** — the failure signature the banner exists for — with `not_in_eob` reading "None of the billed emergency department services… appear on the provided Explanation of Benefits, which is for a different provider and date of service" |
| E7 | 2026-09-13 | agent | ✓ re-run destructively on the real account, snapshot first. Landed on **"Set up your plan"** with the **first-visit** skip wording "Skip for now — audit a bill first", not the origin-aware variant. Firestore after: **0 audits, 0 trackers, 0 eobs, plan gone**. `meta/usage` **byte-identical** before and after (`count:1, planCount:1`, same day strings) — reset is not a way to buy more audits. Everything below in this pass ran on the clean account it produced |
| X1 | 2026-09-13 | agent | ✓ re-verified **three paths and all three dismissal routes** through Claude in Chrome, which the old note said was impossible. delete an audit ("Delete this audit?" / "Your other audits are untouched." / red "Delete audit") · remove the plan ("Remove your plan?" / names the plan and the 10 limits / red "Remove plan") · stop tracking a limit. **Cancel**, **Esc** and **backdrop click** each declined, with `account-snapshot.py verify` returning `ROUND TRIP: EXACT` after each. Remaining paths — delete a saved EOB, replace with an older plan — not re-run today |
| R2 | 2026-09-09 | agent+you | ✓ steps 2-4: bubble present on bills, form and reports; fourth card reads exactly "Worth disputing"; the logo returns to the list from every screen and "New audit" reaches the form. Every fresh load of /app landed on **Bills & coverage**, never the audit form. Step 1 attested by you 2026-09-10 — "login is fine, i did it many times" — from your own repeated use, not verified by the agent. The sub-second "no flash of *No bills audited yet*" assertion in particular has never been checked frame by frame |
| F1 | 2026-09-13 | agent | ✓ re-run from a **report**. Bubble → card ("Send feedback", Bug/Idea/Other, textarea, Send) → "**Thanks — we read every note.**" → textarea cleared and the card closed itself. Firestore `feedback` doc confirmed: `category: bug`, `screen: "report"`, **`auditId: GNRvcvoP79beEX87ZUlV`**, `platform: web` — the auditId attaches only where there is one |
| A1 | 2026-09-10 | you+agent | ✓ **run at last**, split by who could do what. You created a fresh password account: submitting landed on **Confirm your email**, not onboarding, and "I've confirmed it — continue" **refused before the link was opened** — the gate is real, not just a screen. Server side confirms it produced `emailVerified: true` on a `password` provider. Step 3: `lastLoginAt` moved 23:14:59 → 23:27:09, so a genuine re-authentication, and it landed on onboarding with **no second confirm screen**. That screen also read "Skip for now — audit a bill first" — the first-visit wording, which independently corroborates E7's claim to restore a first-visit state, since here it came from an actually new account. Step 5 ✓ by source: no `prompt()` anywhere, no `sendSignInLinkToEmail`/`signInWithEmailLink`, exactly two routes. Step 6 ✓: wrong password and an address with no account produced the **identical** sentence, "That email and password don't match. Check both, or reset your password.", with no raw `Firebase: Error (auth/…)`. Both also showed "No account yet? Create one with this email." — correct, not a bug: `AMBIGUOUS_CREDENTIAL` is exactly the codes where the app cannot tell the two apart, so offering both paths is what enumeration protection requires; showing it for only one would be the leak. Step 4 (Google never sees the confirm screen) ✓ by a full day's use of the Google account. **Not covered:** step 2's "Send it again" and "Sign out and start again" sub-checkpoints. Step 1's hidden "Forgot password?" is now a browser test rather than something anyone has to remember to look at |
| FAM2 | 2026-09-09 | agent | ✓ **no longer blocked.** With `real-sbc/cms-2019.pdf` on file ($500 individual / $1,000 family) and the family EOB as the newest snapshot: deductible **$640.00 of $1,000.00** — the family figure, not the $500 individual one — bar at ~64%, **no conflict warning**, and the card names its choice: "**Family target** from your plan (SBC)". Out-of-pocket resolved the same way, **$640.00 of $5,000.00 · 13%**, family not the $2,500 individual. The per-claim-sum note is the expected one |
...
| S2 | 2026-09-09 | agent | ✓ both documents as images: $2,115.00 / $841.75 / $186.35 — **identical to the same documents as text PDFs**, which is the whole assertion. `duplicate_charge` $145.50 + `billed_vs_allowed` $658.65 + `cost_share_error` $18.00. Worth disputing $842.40 rather than the recorded $836.00, which tracks plan state |
| S3 | 2026-09-09 | agent | ✓ scanned SBC extracted identically to the PDF — Acme Silver PPO, $1,500 deductible, $6,000 OOP. **Both trackers survived the Replace** (Rehabilitation 1/20, Outpatient mental health 5/6), so the regression this plan originally found — Replace discarding confirmed tracker codes — has not returned |
| S4 | 2026-09-09 | agent | ✓ 5 PNGs, five pills, all render; older-plan guard fired and asked before replacing (teal "Replace anyway", not red — X1's non-destructive case, live). Extraction spanned pages: **$500 deductible + $2,500 OOP** from p1, **60-visit caps**, and **"Children's eye exam" 0/1** from p3. Plan-year warning "Your plan year ended 2025-12-31" fired correctly for a 2025 document |
| S1 | 2026-09-07 | agent | ✓ covered by S2's image bill; the removed scan banner stayed removed, review explains pages |
| P1 | 2026-09-09 | agent | `cms-2025.pdf` (earlier), **`cms-2019.pdf`** ✓ ($500 individual / $2,500 OOP, home health 60/yr, rehab 60/yr, children's glasses 1/yr — the SBC FAM2 runs against) and **`cms-older.pdf`** ✓ (extracted its plan year as 2017-01-01 → 2017-12-31, and the older-plan guard compared it correctly against the 2022 plan on file; declined with **Cancel**, which left the plan untouched — X1's dismissal path, live). **`dol-sample-2` run through the UI 2026-09-10** ✓ — $500 deductible / $2,500 OOP, out-of-period banner "Your plan year ended 2018-12-31 — upload your new SBC.", SBC trackers re-derived with CPT hints (home health 0/60, skilled nursing 0/60, rehabilitation 1/60). The older-plan dialog quoted both periods correctly. **It cost two plan uploads for one document** — `planCount` 1 → 3 — because "Replace anyway" re-ran the whole extraction, discarding a plan the model had already read and been paid for. Fixed (`pendingplan.js` parks the extraction, the confirm collects it; hash-matched, single-use, 15-minute TTL so `force` alone cannot skip the cap) and E7 now sweeps the whole `plan` collection so the parked copy cannot survive "erase everything". Verified 2026-09-10 against the Firestore emulator through the real `extractPlan` handler: a force with a parked extraction stores the plan the model already read, moves `planCount` by **zero**, and clears the park so it cannot be replayed. Proven to bite by disabling the shortcut — the test then failed and took 3s, the second model call it exists to prevent. **`dol-sample-3` run through the UI 2026-09-10** ✓ — $500 / $2,500, out-of-period banner for 2022-12-31, four SBC trackers with CPT hints, and **one** plan upload for one document. It earns its place: `pdftotext` renders its "overall deductible?" row with no dollar figures beside the question, and extraction returned $500 / $1,000 anyway. **The double-charge fix then verified on production**, not only against the emulator: with `dol-sample-3` (2022) on file, uploading `dol-sample-2` (2018) raised the older-plan dialog and **"Replace anyway" left `planCount` at 2** — the same journey that moved it 1 → 3 earlier the same day. All five government specimens are now covered end to end; the gap they cannot close is real plan variation |
| G1 | 2026-09-10 | agent+you | ✓ kill switch run live (you flipped `auditsEnabled`, the write is blocked for the agent by design). **Found a regression:** `analyze` returned **503** with the guard's "Audits are paused right now" and the client showed "Analysis failed — please try again" — advice that cannot work against a deliberate pause. Four call sites tested `resource-exhausted` and none tested `unavailable`, so the daily-allowance copy reached members and both kill-switch messages did not. Fixed, deployed, and re-verified **with the guard still off**: the form now reads "Audits are paused right now. Please try again later." **A second defect fell out of the same run:** the daily allowance is claimed *before* the spend guard is consulted, so a refused attempt still cost one of the member's ten — and the message invited a retry, which cost another. Both limiters now return a `refund()` that the call site fires when `reserveModelCall` throws (`functions/ratelimit.js`, six unit tests). Re-verified 2026-09-10 with the guard off: **three** refused attempts in a row left `count` at 6, while the usage document's `updateTime` advanced on each — so every attempt was charged and refunded, not merely rejected before the charge. A normal audit either side moved `count` by exactly one. **Restore verified**, which is the leg that makes the whole plan worth running: with `auditsEnabled` back to `true`, the very same t1 pair that had just been refused three times returned $175.00 / $120.00 / $120.00 / **$55.00** and advanced the tracker to visit 3 of 6, `count` 6 → 7. **Independence leg** (2026-09-10): with `plansEnabled: false` and audits left on, the SBC upload was refused with "Plan uploads are paused right now. Please try again later.", `planCount` held at 1 with `updateTime` advancing — the **plan-side** refund's first live proof — and an audit ran normally throughout, so the two switches are genuinely independent. Note `fake-sbc.pdf` cannot test this: the duplicate check fires before the guard, so use a plan not already on file. **Ceiling leg** (2026-09-10, `dailyCalls: 1` against a spend of 4): refused with "We've hit today's limit across all users. Please try again tomorrow.", `count` held at 8, and the refund fired here too — so it is not specific to the kill switch. **Found a third defect**, the exact inverse of the first: three client call sites branch on the `resource-exhausted` **code** to open the personal daily-limit dialog, and the global ceiling shares that code. The ceiling therefore showed "That's today's 10 audits… Capping it at 10 a day… Your allowance comes back at 5:00 PM on Thursday" to an account that had used **8** — every number false, and the member's own usage blamed for a budget ceiling. `guard.js` now marks the ceiling `details: { scope: "global" }` and `showLimitIfPersonal()` suppresses the dialog for it; deliberately tolerant, so an older deploy sending no details still shows the dialog rather than silently hiding a real cap. Fixed, deployed, re-verified with the ceiling still at 1. Both restores proven by re-running the same pair that had just been refused |
| PAY1 | 2026-09-13 | agent | **Still dormant, and it degrades correctly.** "Generate dispute email" produced no paywall and **no raw error** — a card reading "Appeal letters are coming soon", what the letter would do, and "**Nothing here is blocked in the meantime**" pointing at Print / save PDF. PAY1's load-bearing assertion holds either way: **no letter text anywhere on the page**. The card steps stay tier 3 |
| REV1 | 2026-09-13 | agent | ✓ steps 1 and 4 re-run. One-page bill: "This is what we send", page shown large, **no page pills**. Saved EOB: "This is a saved EOB. Its text was stored when you first uploaded it — the file itself was not, so there are no pages to show." Steps 2-3 (multi-page pills, .txt/.html) not re-run this pass — though C6 exercised pills at 24 pages |
...
*FAM2 ran on 2026-09-09 and passed — removed from this table.*
...
`functions/test/integration.test.js` runs the **real exported handlers** against
the Firestore emulator, so the wiring inside `analyze()` and `extractPlan()` is
covered rather than only the modules they call.

    npm --prefix functions run emulators     # needs a JDK; brew install openjdk
    npm --prefix functions run test:integration

It skips rather than fails when the emulator is not running, because a suite that
goes red on a laptop without Java trains people to ignore it.

**This is the layer whose absence let bugs ship.** Every defect on 2026-09-10
needed server state flipped — a guard switch, a counter, a parked extraction —
and then an outcome checked. Unit tests cannot flip state and the browser suite
never reaches the functions, so the only thing that could do it was a person
pasting `curl` at a production database. That is why `pendingplan` shipped
unverified, and it is what this replaces.
