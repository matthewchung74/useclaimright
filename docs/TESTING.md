...
| E4 | 2026-09-10 | agent | ✓ "This doesn't look like a Summary of Benefits." The plan on file survived untouched (Acme Silver PPO, 2026-01-01 → 2026-12-31). Unlike M6 this one **does** reach the model, and `planCount` moved by one. **Re-run after the G1 copy fix** rewrote how server messages reach the form: this is the one plan that proves an `invalid-argument` message still arrives verbatim, rather than being flattened to the generic "Plan extraction failed" |
