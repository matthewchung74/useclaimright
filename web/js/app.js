...
$("confirm-review").onclick = async () => {
  ...
  try {
    ...
    renderReport(data, { ocrLow, model: data.model, planApplied: data.planApplied, planReason: data.planReason,
      // NOT `!payload.eob`. asDoc(null) returns { text: "", images: [] } — an
      // object, and objects are truthy — so that test was false for every audit
      // ever run and this branch never once fired on a fresh report. Someone
      // who ticked "I don't have an EOB" was told "the bill and EOB appear
      // consistent" about a comparison that never ran. The copy fixed on
      // 2026-09-09 was correct; nothing ever reached it. Found on production
      // 2026-09-13 running E2.
      //
      // Judge it the way openAudit does, on what the model transcribed: no EOB
      // text means there was no EOB. That answer is also right for a saved EOB
      // pulled from the library, which `state.eob` would miss.
